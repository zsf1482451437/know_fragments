...
function appendLog(state, actor, action, target, detail) {
    state.logs.unshift(createOperationLog({ actor, action, target, detail }));
}
function getRecordLogDetail(record) {
    return `${record.title || '未命名'} ${record.amount}`;
}
function appendRecordLog(state, actor, action, record) {
...
router.post('/records', async (ctx) => {
    const state = await readState();
    const record = createRecord(ctx.request.body, ctx.state.userName);
    state.records.unshift(record);
    appendRecordLog(state, ctx.state.userName, '新增流水', record);
    ctx.status = 201;
...
router.patch('/records/:id', async (ctx) => {
    ...
    appendRecordLog(state, ctx.state.userName, '编辑流水', record);
    ctx.body = await writeState(state);
});
router.delete('/records/:id', async (ctx) => {
    ...
    appendRecordLog(state, ctx.state.userName, '删除流水', record);
    ctx.body = await writeState(state);
});
app.use(bodyParser());
app.use(requireAuth);
