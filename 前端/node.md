...
```bash
npm config set @reolink-services:registry 'https://npm.reolink.dev'
```

1. **设置私有包仓库**
   
   - 为以 `@reolink-services` 开头的 npm 包设置专用的私有仓库地址
   
   - 当安装或发布 `@reolink-services/*` 这类包时，npm 会使用指定的私有仓库

```bash
npm adduser --auth-type=legacy --registry 'https://npm.reolink.dev'
```

1. **登录私有仓库**
   
   - 在 Reolink 的私有 npm 仓库 (`https://npm.reolink.dev`) 上进行身份验证
   
   - 建立与私有仓库的安全连接

2. **认证方式**
   
   - `--auth-type=legacy`：使用传统的用户名/密码认证方式
   
   - 而不是较新的基于 token 或 OAuth 的认证方式

## pnpm

**性能优势**

- 磁盘空间：使用硬链接和符号链接，相同依赖只存储一份，节省空间
...
2. 在 CI/CD 中使用 pnpm install --frozen-lockfile

3. 使用 pnpm-lock.yaml 锁定版本

4. 在 monorepo 中使用 workspace

### 幽灵依赖

代码使用了某个包，但这个包并没有直接声明在你的 package.json 的 dependencies 中，而是作为其他依赖的子依赖存在。
...
- express 依赖 lodash（作为子依赖）

- express 依赖 cookie-parser（作为子依赖）
...
node_modules/
├── express/          # 直接安装的
├── lodash/           # express 的依赖，但被提升到根目录
└── cookie-parser/    # express 的依赖，但被提升到根目录
```
...
```js
// index.js
const lodash = require('lodash');  // ❌ 幽灵依赖！
...
1. 你的 package.json 中没有声明 lodash 和 cookie-parser

2. 但代码可以正常运行，因为它们被提升到了 node_modules 根目录

3. 如果 express 升级后不再依赖 lodash，你的代码就会突然报错

4. 如果其他人克隆项目后安装，可能因为依赖解析顺序不同而失败
...
│   ├── lodash@4.17.21/
│   └── cookie-parser@1.4.6/
├── express/                  # 符号链接 → .pnpm/express@4.18.0/
└── .modules.yaml             # 模块映射文件
```
...
```js
// index.js
const lodash = require('lodash');  // ❌ 报错！找不到模块
```
...
1. 搜索代码中所有的 require() 和 import

2. 检查这些包是否都在 package.json 中声明

3. 如果使用了但没声明，就是幽灵依赖

### 最佳实践

1. 使用 pnpm 的严格模式，避免幽灵依赖

2. 如果必须使用某个包，在 package.json 中显式声明

3. 定期使用 depcheck 检查依赖

4. 在 CI/CD 中使用 pnpm install --frozen-lockfile 确保一致性
