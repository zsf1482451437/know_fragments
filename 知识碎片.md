...
一个用户，需要访问三个不同的网站：网站A、网站B和网站C。这些网站都实现了单点登录功能。

1. 你首先访问**网站A**，并尝试进行登录。由于你还没有进行身份验证，网站A将**重定向**你到一个**认证服务器**，例如Auth Server。
2. 在**Auth Server**上，你被要求提供你的**凭据**（例如用户名和密码）进行身份验证。一旦你的凭据被验证通过，Auth Server会创建一个**授权令牌**（Authorization Token）。
3. Auth Server将**授权令牌**返回给网站A，并告知网站A你已成功通过身份验证。**网站A**将在你的浏览器中设置一个**会话**（Session）以维持登录状态。
4. 现在你想访问**网站B**。你直接在浏览器中输入网站B的地址。由于你之前已经进行了单点登录，网站B会检测到你的登录状态，并发送一个**认证请求**到Auth Server。
5. Auth Server注意到你已经通过身份验证，并且仍然拥有有效的会话。因此，Auth Server会生成一个**令牌**（Token）并将其发送给网站B。
6. 网站B接收到令牌后，验证令牌的有效性。一旦验证通过，网站B将创建一个会话，并将你重定向到网站B的首页。现在你已经成功单点登录到网站B。
7. 类似地，当你访问网站C时，网站C也会发送认证请求到Auth Server，并通过验证后向你提供访问权限。

**结果**

可以看到，单点登录的原理是在**多个网站之间共享认证状态**。一旦你在一个网站上进行了身份验证，其他网站可以通过认证服务器验证你的身份，并无需再次输入凭据。这样，你可以在多个网站上无缝地切换而无需重复登录。

**关键词**

重定向、认证服务器、凭据、授权令牌、会话、认证请求
...
2.**OAuth 2.0 和 OpenID Connect**协议的认证服务器返回的令牌有：

- 访问令牌（Access Token）
- 刷新令牌（Refresh Token）
- 身份令牌（ID Token）

而使用**SAML**协议，网站通常会收到一个 SAML 断言（Assertion），其中包含有关用户身份验证的信息。网站可以解析和处理 SAML 断言，以获取用户的身份和其他属性。

**过程是这样的：**

当开始单点登录授权时，会去Authorization URL

配置的client id、client secret、authorization url、token url、userinfo url、jwk set url用途是这样的：

1. 去**authorization url**，根据**client id**、**client secret**返回一个授权码**code**
2. 去**token url**，使用**code**去请求**access token**；
3. 去**jwk set url**，验证**access token**；
4. 去**userinfo url**，使用**access token**请求用户信息；

获取**访问令牌**后，它需要验证该访问令牌**是否有效**、**是否被篡改**过，并且确保该令牌是**由授权服务器签发**的。这是为了防止伪造令牌或使用无效令牌进行未经授权的访问。

JWK（JSON Web Key）是一种用于加密和验证数字签名的标准格式，通常与JWT（JSON Web Token）一起使用。**JWK集合**是一组包含公钥和其他相关信息的JSON对象，用于解析和验证JWT的签名。

在OAuth 2.0的流程中，**JWK Set URL**提供了第三方应用所需的**JWK集合**。第三方应用可以使用这个URL来获取用于验证和解析JWT的**公钥**。然后，第三方应用可以使用公钥来验证访问令牌的**数字签名**，以确保令牌的合法性和完整性。
